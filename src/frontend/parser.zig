...
pub fn parse(allocator: std.mem.Allocator, tokens: *const std.ArrayList(Token)) Error!std.ArrayList(ast.Statement) {
    var parser = Self{ .allocator = allocator, .tokens = tokens, .current_token = tokens.items[0] };
...
fn parse_statement(self: *Self, previous_attributes: ?std.ArrayList(ast.Statement.Attribute)) Error!?ast.Statement {
    ...
    return switch (self.current_token) {
        ...
        .semi_colon => blk: {
            ...
            const expr = try self.parse_expression(false);
            self.eat(.semi_colon);
...
fn parse_function(self: *Self, attributes: std.ArrayList(ast.Statement.Attribute)) Error!ast.Statement {
    ...
    while (!self.check(.rparent)) {
        const arg_name = try self.parse_primitive(false);
        var type_expr: ?ast.Expr = null;
        if (self.check(.colon)) {
            self.eat(.colon);
            type_expr = try self.parse_expression(true);
        }
...
fn parse_var_declaration(self: *Self, attributes: std.ArrayList(ast.Statement.Attribute)) Error!ast.Statement {
    ...
    self.eat(.eq);
    const value = try self.parse_expression(false);
    self.eat(.semi_colon);
...
fn parse_type(self: *Self, attributes: std.ArrayList(ast.Statement.Attribute)) Error!ast.Statement {
    ...
    self.skip();
    self.eat(.eq);
    const value = try self.parse_expression(true);
    self.eat(.semi_colon);
...
fn parse_expression(self: *Self, for_type: bool) Error!ast.Expr {
    return self.parse_assign(for_type);
}

/// Parse primitve value like number, string or more
fn parse_primitive(self: *Self, for_type: bool) Error!ast.Expr {
    return switch (self.current_token) {
        .ident => |ident| blk: {
            self.skip();
        ...
        .lparent => blk: {
            self.skip();
            const expr = try self.parse_expression(for_type);
            self.eat(.rparent);
            break :blk .{ .parent = try Bin(ast.Expr).init(self.allocator, expr) };
        },
        .question => blk: {
            self.skip();
            break :blk .{ .optional = try Bin(ast.Expr).init(self.allocator, try self.parse_expression(for_type)) };
        },
        else => unreachable,
    };
}
...
fn parse_unary(self: *Self, for_type: bool) Error!ast.Expr {
    if (self.check(Token.plus) or self.check(Token.minus)) {
        const op = ast.UnaryOp.from_token(self.current_token);
        self.skip();
        const right = try self.parse_unary(for_type);

        return .{ .unary_operation = .{ .right = try Bin(ast.Expr).init(self.allocator, right), .kind = op } };
    }

    return self.parse_primitive(for_type);
}

/// Parse expression like `2 * 3`
fn parse_factor(self: *Self, for_type: bool) Error!ast.Expr {
    var left = try self.parse_unary(for_type);
...
/// Parse expression like `2 + 3`
fn parse_term(self: *Self, for_type: bool) Error!ast.Expr {
    var left = try self.parse_factor(for_type);
    while (self.check(Token.plus) or self.check(Token.minus)) {
        const op = ast.BinaryOp.from_token(self.current_token);
        self.skip();
        const right = try self.parse_factor(for_type);
        left = .{ .binary_operation = .{
            .left = try Bin(ast.Expr).init(self.allocator, left),
            .right = try Bin(ast.Expr).init(self.allocator, right),
            .kind = op,
...
fn parse_condition(self: *Self, for_type: bool) Error!ast.Expr {
    var left = try self.parse_term(for_type);
    const token_int = @intFromEnum(self.current_token);
    while (token_int >= @intFromEnum(Token.eqs) and token_int <= @intFromEnum(Token.lt)) {
        const op = ast.BinaryOp.from_token(self.current_token);
        self.skip();
        const right = try self.parse_term(for_type);
        left = .{ .binary_operation = .{
            .left = try Bin(ast.Expr).init(self.allocator, left),
            .right = try Bin(ast.Expr).init(self.allocator, right),
            .kind = op,
...
fn parse_assign(self: *Self, for_type: bool) Error!ast.Expr {
    var left = try self.parse_condition(for_type);
    const token_int = @intFromEnum(self.current_token);
    while (!for_type and token_int >= @intFromEnum(Token.eq) and token_int <= @intFromEnum(Token.conditional_eq)) {
        const op = ast.BinaryOp.from_token(self.current_token);
        self.skip();
        const right = try self.parse_condition(for_type);
        left = .{ .binary_operation = .{
            .left = try Bin(ast.Expr).init(self.allocator, left),
            .right = try Bin(ast.Expr).init(self.allocator, right),
            .kind = op,
...
fn parse_test(source: lexer.Source, allocator: std.mem.Allocator) !std.ArrayList(ast.Statement) {
...
test "parser parse variable declaration" {
    ...
    const input = "let a = 0;";
    const result = try parse_test(.{ .buffer = input, .file_name = "test_file" }, allocator);
    defer {
        for (result.items) |s| {
            s.deinit();
        }
...
test "parser parse null" {
    const allocator = std.testing.allocator;
    const input = "null;";
    var result = try parse_test(.{ .buffer = input, .file_name = "test_file" }, allocator);
    defer {
        for (result.items) |s| {
            s.deinit();
        }
